 ...
 */
class ArrayVariant : public CollectionVariant {
 ...
 protected:
   ...
   */
  class Value : public CollectionVariant::Value {
   public:
    /** \brief Default constructor
     */
    Value();
     ...
     *   (implementation)
     */
    void setMember(const std::string& name, const Variant& member) override;
     ...
     *   (implementation)
     */
    Variant getMember(const std::string& name) const override;
     ...
     */
    void writeMember(std::ostream& stream, int index) const override;
  ...
  class ValueImplV : public Value {
   public:
     ...
     *   (implementation)
     */
    void setMember(int index, const Variant& member) override;

    /** \brief Retrieve a member of the variant collection by index
     *   (implementation)
     */
    Variant getMember(int index) const override;

    /** \brief Add a member to the array (implementation)
     */
    void addMember(const Variant& member) override;

    /** \brief Resize the array (implementation)
     */
    void resize(size_t numMembers) override;
   ...
   */
  template <typename T>
  class ValueImplT : public Variant::ValueT<typename type_traits::ArrayType<T>::ValueType>, public Value {
   public:
     ...
     */
    using MemberType = typename type_traits::ArrayType<T>::MemberType;
   ...
   */
  template <typename T>
  static void initialize(typename type_traits::ArrayType<T>::ValueType& array,
                         typename boost::enable_if<typename type_traits::ArrayType<T>::IsDynamic>::type* /*unused*/ = 0);
  ...
  template <typename T>
  static void initialize(typename type_traits::ArrayType<T>::ValueType& array,
                         typename boost::disable_if<typename type_traits::ArrayType<T>::IsDynamic>::type* /*unused*/ = 0);

  /** \brief Add a member to an array (overloaded version taking a
   *   dynamic array)
   */
  template <typename T>
  static void add(typename type_traits::ArrayType<T>::ValueType& array, const typename type_traits::ArrayType<T>::MemberType& member,
                  typename boost::enable_if<typename type_traits::ArrayType<T>::IsDynamic>::type* /*unused*/ = 0);
   ...
   */
  template <typename T>
  static void resize(typename type_traits::ArrayType<T>::ValueType& array, size_t numMembers,
                     typename boost::enable_if<typename type_traits::ArrayType<T>::IsDynamic>::type* /*unused*/ = 0);
   ...
   */
  template <typename T>
  static void clear(typename type_traits::ArrayType<T>::ValueType& array,
                    typename boost::enable_if<typename type_traits::ArrayType<T>::IsDynamic>::type* /*unused*/ = 0);

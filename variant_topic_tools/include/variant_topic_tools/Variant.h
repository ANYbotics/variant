...
class Variant {
 ...
 public:
   ...
   */
  template <typename T>
  typename type_traits::DataType<T>::ValueType& getValue();

  /** \brief Retrieve the variant's value (const version)
   */
  template <typename T>
  const typename type_traits::DataType<T>::ValueType& getValue() const;
  ...
  /** \brief True, if this variant's value equals another value
   */
  template <typename T>
  bool operator==(const T& value) const;

  /** \brief True, if this variant's value does not equal another value
   */
  template <typename T>
  bool operator!=(const T& value) const;
 ...
 protected:
   ...
   */
  using ValuePtr = boost::shared_ptr<Value>;

  /** \brief Declaration of the variant value weak pointer type
   */
  using ValueWPtr = boost::weak_ptr<Value>;
   ...
   */
  template <typename T>
  class ValueT : public virtual Value {
   public:
    /** \brief Default constructor
     */
    ValueT();
   ...
   */
  template <typename T>
  static void setValue(Variant& dst, const T& value, typename boost::enable_if<boost::is_base_of<Variant, T> >::type* /*unused*/ = 0);

  /** \brief Set a variant's value (overloaded version taking a variant
   *   value)
   */
  template <typename T>
  static void setValue(Variant& dst, const T& value, typename boost::disable_if<boost::is_base_of<Variant, T> >::type* /*unused*/ = 0);

  /** \brief True, if a variant's value equals another value (overloaded
   *   version taking a variant)
   */
  template <typename T>
  static bool isEqual(const Variant& variant, const T& value,
                      typename boost::enable_if<boost::is_base_of<Variant, T> >::type* /*unused*/ = 0);

  /** \brief True, if a variant's value equals another value (overloaded
   *   version taking a variant value)
   */
  template <typename T>
  static bool isEqual(const Variant& variant, const T& value,
                      typename boost::disable_if<boost::is_base_of<Variant, T> >::type* /*unused*/ = 0);

  /** \brief Assign a variant (overloaded version taking a variant)
   */
  template <typename T>
  static void assign(Variant& dst, const T& src, typename boost::enable_if<boost::is_base_of<Variant, T> >::type* /*unused*/ = 0);

  /** \brief Assign a variant (overloaded version taking a variant value)
   */
  template <typename T>
  static void assign(Variant& dst, const T& src, typename boost::disable_if<boost::is_base_of<Variant, T> >::type* /*unused*/ = 0);
};

...
class DataType {
 ...
 public:
   ...
   */
  inline operator void*() const { return (impl) ? (void*)1 : (void*)nullptr; };

  /** \brief Equality comparison operator
   */
  inline bool operator==(const DataType& dataType) const {
    return (impl && dataType.impl) ? (*impl == *dataType.impl) : (impl == dataType.impl);
  };
 ...
 protected:
   ...
   */
  using ImplPtr = boost::shared_ptr<boost::shared_ptr<Impl>>;

  /** \brief Declaration of the data type implementation weak pointer
   *   type
   */
  using ImplWPtr = boost::shared_ptr<boost::weak_ptr<Impl>>;

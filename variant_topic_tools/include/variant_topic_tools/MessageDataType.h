 ...
 */
class MessageDataType : public DataType {
 ...
 public:
   ...
   */
  const MessageMember& getMember(int index) const;
   ...
   */
  const MessageConstant& getConstantMember(int index) const;
   ...
   */
  const MessageVariable& getVariableMember(int index) const;
   ...
   */
  MessageDataType& operator=(const DataType& src) override;
 ...
 protected:
  ...
  class ImplV : public Impl {
   public:
     ...
     */
    ImplV(std::string identifier, const MessageFieldCollection<MessageConstant>& constantMembers,
     ...
     *   (implementation)
     */
    std::string getMD5Sum() const override;

    /** \brief Retrieve the definition this message data type
     *   (implementation)
     */
    const std::string& getDefinition() const override;
     ...
     *   (implementation)
     */
    void addConstantMember(const MessageConstant& member) override;

    /** \brief Add a variable member to this message data type
     *   (implementation)
     */
    void addVariableMember(const MessageVariable& member) override;
   ...
   *   sequence of constant members, and a sequence of variable members)
   */
  MessageDataType(const std::string& identifier, const MessageFieldCollection<MessageConstant>& constantMembers,
                  const MessageFieldCollection<MessageVariable>& variableMembers);
  ...
  static void addMember(MessageVariable& member, size_t offset, typename boost::enable_if<type_traits::IsArray<M> >::type* /*unused*/ = 0,
                        typename boost::enable_if<typename type_traits::ArrayType<M>::IsDynamic>::type* /*unused*/ = 0);
  ...
  template <typename T, typename M>
  static void addMember(MessageVariable& member, size_t offset, typename boost::enable_if<type_traits::IsArray<M> >::type* /*unused*/ = 0,
                        typename boost::disable_if<typename type_traits::ArrayType<M>::IsDynamic>::type* /*unused*/ = 0);

  /** \brief Add a variable message member (overloaded version for adding
   *   a non-array member)
   */
  template <typename T, typename M>
  static void addMember(MessageVariable& member, size_t offset, typename boost::disable_if<type_traits::IsArray<M> >::type* /*unused*/ = 0);
};
}  // namespace variant_topic_tools

...
void connectCallback(const ros::SingleSubscriberPublisher& /*unused*/);
...
void subscribe() {
  subscriber = nodeHandle->subscribe(subscriberTopic, subscriberQueueSize, &callback, subscriberTransportHints);
}

void connectCallback(const ros::SingleSubscriberPublisher& /*unused*/) {
  if (lazy && (subscriber == nullptr)) {
    subscribe();
  }
}
...
void callback(const ros::MessageEvent<variant_topic_tools::Message>& messageEvent) {
  ...
  boost::shared_ptr<const ros::M_string> connectionHeader = messageEvent.getConnectionHeaderPtr();
  ...
  if (publisher == nullptr) {
    ...
    ros::AdvertiseOptions options(publisherTopic, publisherQueueSize, message->getType().getMD5Sum(), message->getType().getDataType(),
                                  message->getType().getDefinition(), connectCallback);
    options.latch = latch;

    publisher = nodeHandle->advertise<variant_msgs::Variant>(publisherTopic, publisherQueueSize, connectCallback,
                                                             ros::SubscriberStatusCallback(), ros::VoidConstPtr(), latch);
  }

  if (!lazy || (publisher.getNumSubscribers() != 0u)) {
    boost::shared_ptr<const variant_msgs::Variant> variantMessage = message->toVariantMessage();
...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, publisherTopic + "_relay", ros::init_options::AnonymousName);

...
TEST(Message, Conversion) {
  ...
  EXPECT_EQ(ros::message_traits::datatype<variant_msgs::Test>(), m2.getType().getDataType());
  EXPECT_EQ(ros::message_traits::md5sum<variant_msgs::Test>(), m2.getType().getMD5Sum());
  EXPECT_EQ(ros::message_traits::definition<variant_msgs::Test>(), m2.getType().getDefinition());
  ...
  Message m5 = m4;
  EXPECT_EQ(ros::message_traits::datatype<geometry_msgs::PoseStamped>(), m5.getType().getDataType());
  EXPECT_EQ(ros::message_traits::md5sum<geometry_msgs::PoseStamped>(), m5.getType().getMD5Sum());
  EXPECT_EQ(ros::message_traits::definition<geometry_msgs::PoseStamped>(), m5.getType().getDefinition());
...
TEST(Message, Serialization) {
  ...
  EXPECT_EQ(ros::serialization::serializationLength<variant_msgs::Test>(m1), m3.getData().size());
